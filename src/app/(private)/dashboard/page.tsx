...
export default function DashboardAllCompanies() {
  ...
  const latestProductInfo = useMemo<{
    latest: Product | null;
    latestCompany: CompanyWithProducts | null;
  }>(() => {
    ...
    companies.forEach((company) => {
      company.products.forEach((prod) => {
        if (!latest || new Date(prod.date_receipt) > new Date(latest.date_receipt)) {
          latest = prod;
    ...
    datasets: [
      {
        ...
        data: companies.map((company) =>
          company.products.reduce((sum, prod) => sum + (prod.price ?? 0), 0)
        ),
